...
description: Orquestador principal del pipeline AGENTE PAGINA WEB. Recibe la petición del usuario, valida que exista brief del cliente, decide el stack, y delega en cascada a los agentes especializados (brand-researcher → site-architect → design-director → mockup-builder → code-gen → qa-reviewer → kb-writer). Mantiene estado en output/<cliente>/ y reporta progreso al usuario. INVOCAR cuando el usuario pida "genera la página web para <cliente>", "crea un nuevo proyecto web", "arma el sitio de <cliente>", o variaciones.
